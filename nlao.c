...
void nLAOstar(int gThread)
{
  ...
  for (node = Start; 
       ...
       node = node->BestAction->NextState->State)
    {
      ...
      path[NumExpandedStatesIter] = node->StateNo;
      Backup( node );
      NumExpandedStatesIter++;
      NumExpandedStates++;
    }
  
  best_length = NumExpandedStatesIter;  
  //t = (pthread_t*)malloc(best_length * sizeof(pthread_t));
  
  printf("\nNum expanded states along most probable path to goal: %d",
	 NumExpandedStatesIter);
  ...
  for (Iteration = 1; ;Iteration++) {
    ...
    /*for (t_num = 1; t_num < best_length; t_num++) {
      //printf("\nThread ID: %d", t_num);
      if (t_num < 2)
        pthread_create(&t[t_num], NULL, (void*)(ExpandSolution), (void*)(StateIndex[path[t_num]]));
      else
        pthread_create(&t[t_num], NULL, (void*)(rExpandSolution), (void*)(StateIndex[path[t_num]]));
    }*/
    for (t_num = 1; t_num < gThread; t_num++) {
      //printf("\nThread ID: %d", t_num);
      pos = t_num * best_length / gThread;
      if (t_num < 2)
        pthread_create(&t[t_num], NULL, (void*)(ExpandSolution), (void*)(StateIndex[path[t_num]]));
      else
        pthread_create(&t[t_num], NULL, (void*)(rExpandSolution), (void*)(StateIndex[path[pos]]));
    }
