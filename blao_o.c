...
void bLAOOstar(void)
{
  ...
  for (Iteration = 1; ;Iteration++) {
    ...
    /* Create two threads and let each thread handle each one */
    pthread_create(&t1,NULL,(void *)(ExpandSolution),(void *)Start);
    pthread_create(&t2,NULL,(void *)(rExpandSolution),(void *)Goal);
...

{
  ...
  if ((node->Expanded == 0) && (node->Terminal != 5)) {
    ExpandNode(node);
...
//static struct StateListNode *DisplaySolutionRecursive(struct StateNode *node)
static void DisplaySolutionRecursive(struct StateNode *node)/* Set NumSolutionStates to zero before calling this recursive function */
{
  struct StateDistribution *successor;
