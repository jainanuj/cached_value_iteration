...
void LAOstar(void)
{
  ...
  for (node = Start; 
       (node->Terminal != 1) && (node->Terminal != 5) && (node->Expanded < 1); 
  ...
  printf("\nDeterministic shortest path %d", (int)Start->f);
  if ( node->Terminal == 1 )
    printf("\nNum expanded states along most probable path to goal: %d", NumExpandedStatesIter);
  else
    printf("\nNum expanded states along most probable path to terminal: %d", NumExpandedStatesIter);

  for (Iteration = 1; ;Iteration++) {

//    printf("\n%d ( %f secs.)", Iteration, (float)(clock()-gStartTime)/CLOCKS_PER_SEC);
//    printf("  f: %f", Start->f);
    ...
    /* Call convergence test if solution graph has no unexpanded tip states */
      //ANUJ - added () around && - not sure if correct.
    if ( (time > threshold) || ((NumExpandedStatesIter == 0) && ConvergenceTest()) ) {
      printf("\n%d ( %f secs.)", Iteration, (float)(clock()-gStartTime)/CLOCKS_PER_SEC);
      printf("  Converged!");
      break;
    }
...
//      error = Start->meanFirstPassage * Residual;
      // need to use a high epsilon due to incorrect convergence value
//      error = Residual * 10000;
      error = Residual;
...
{
  struct StateDistribution *successor;
  double Diff, time, threshold = 300.0;

  /* Count this node as part of best solution */
  NumSolutionStates++;
  
  /*printf("\nConvergence: %d", node->StateNo);
  printf("\nConvergence: %d", node->StateNo); */
