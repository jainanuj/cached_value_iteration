...
\label{alg:CaV}
\begin{algorithmic}[1]
\ForAll {$\textit{S} \in \textit{SS}$ and $(s,a) \in S$}	\label{alg:CaV:setupStart}
	\ForAll{($s' \mid p(\textit{s'$|$s,a}) > 0$ and $s' \notin \textit{S}$)}
...
\While {!\textit{q.isEmpty()}}	\label{alg:CaV:cachedAlg}
	...
	\ForAll {\textit{(S',s')} $\in$ $ExtSetSt(S,s,a)$}
		\State  $ExtSetSt(Ssa) \gets V[s']$\Comment{\textit{V[s']} = ${\displaystyle\max_{a'\in A}Q(s',a')}$}
		\label{alg:CaV:extStates}
	\EndFor
	\ForAll {\textit{(s,a)} $\in$ \textit{S}}\Comment{Sum external state contributions}
	...
	\State $\textit{Q(s,a)} \leftarrow \textit{R(s,a)}$ + \\
		  $\gamma \sum\limits_{s'_{i}} T(s,a,s'_{i})  {\displaystyle\max_{a'\in A} } Q(s'_{i},a') + e.c_{s,a}$\Comment{$s'_{i} \in S$} 
			\label{alg:CaV:finalEqn}
	\If {$Q(s,a)$ for any (s,a) changes}
		\State $S\_changed = true$
	\EndIf

	\If {\textit{S\_changed}}
		\State \textit{q.enqueue(S.D(S))}\Comment{Add dependent Sets to queue}
	\EndIf
...
\bibitem{b11} David Wingate and Kevin D. Seppi, ``Efficient Value Iteration Using Partitioned Models,'' Proceedings of the International Conference on Machine Learning and Applications - ICMLA '03, pp. 53-59, 2003.
\bibitem{suttonbook} Richard S. Sutton and Andrew G. Barto, ``Reinforcement Learning: An Introduction,'' The MIT Press Cambridge, Massachusetts London, 2nd ed, 2012.
\bibitem{neuro-dyn} Dimitri P. Bertsekas and John N. Tsitsiklis, ``Neuro-Dynamic Programming,'' Athena Scientific, Belmont, Massachusetts, 1996.
\bibitem{b8} D.J White, ``A Survey of Applications of Markov Decision Processes,'' The Journal of the Operational Research Society, vol 44, pp. 1073-1096, 1993.
\bibitem{chunchun-sahni} C. Zhao and S. Sahni, ``Cache and energy efficient algorithms for Nussinov RNA folding,'' BMC Bioinformatics, vol. 18, no. 15 pp. 518, 2017
\bibitem{b9} V.R Rao and L.J Thomas, ``Dynamic Models for Sales promotion policies," The Journal of the Operational Research Society, vol. 24, no. 3, pp 403-407. 1973.

\bibitem{b10} Peng Dai and Mausam and Daniel S. Weld and Judy Goldsmith, ``Topological Value Iteration Algorithms," Journal of Artificial Intelligence Research, vol. 42, pp 181-209, 2011.
\bibitem{wingate_seppi2} D Wingate and Kevin D Seppi, ``Prioritization Methods for accelerating MDP Solvers," Journal of Machine Learning Research, vol. 6, no. 15, pp 851-881, 2005.
\bibitem{perf} ``Perf tool: Profiler tool for Linux 2.6+ based systems," https://perf.wiki.kernel.org/index.php/Tutorial.

\bibitem{ApproxVI} Aaron Sidford, Mengdi Wang, Xian Wu, Yinyu Ye, ``Variance Reduced Value Iteration and Faster Algorithms for Solving Markov Decision Processes,", CoRR, abs/1710.09988, Oct 2017.
\bibitem{Eff_CMDP} Constantine Caramanis, Nedialko B. Dimitrov, and David P. Morton, ``Efficient Algorithms for Budget-Constrained Markov Decision Processes, IEEE Transactions on Automatic Control, vol. 59, no. 10, Oct 2014.
