...
double value_update( world_t *w, int l_part, int l_state )
{
    ...
    printf("Value COMPUED for part:%d, state:%d, action:%d is:%f.\n",l_part,l_state,0,value);
...
double reward_or_value( world_t *w, int l_part, int l_state, int a ) {
    double value, tmp;
    state_t *st;
    int j; double tmpinitval = 0, tmpbufinit;entry_t *et; vec_t *b;
    ...
    if (l_part ==0 && l_state == 1 && a == 0 && w->parts[l_part].convergence_factor >= 10)
        printf("REWORVALUE - Value from ext deps & reward for part:%d, state:%d, action:%d is:%f\n",l_part, l_state,a,value);
